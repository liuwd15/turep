"""FFADVI model wrapper and training-plan customizations for turep."""
...
# from scvi.model.base._archesmixin import _get_loaded_data,_pad_and_sort_query_anndata
...
class SemiSupervisedTrainingPlanFixed(SemiSupervisedTrainingPlan):
    """Semi-supervised training plan with additional logged classification metrics.

    This plan keeps standard scvi training behavior and explicitly logs
    classification loss, accuracy, F1, and calibration error in train/validation
    loops when labeled observations are present.
    """
    ...
    **model_kwargs
    ...
    def __init__(
        ...
        **model_kwargs,
        ...
        # For FFADVI, we pass the full n_labels (including unlabeled) to FFADVAE
        # The external fadvi.FADVAE will handle the unlabeled category internally
        ...
        # Store n_labels (NOT reduced like SCANVI does)
        self.n_labels_original = self.summary_stats.n_labels
        self.n_labels = n_labels  # Full count for FADVAE module
        ...
        self._model_summary_string = (
            ...
            f"focal_alpha: {focal_alpha}, focal_gamma: {focal_gamma}, focal_reduction: {focal_reduction}, "
    ...
    def get_latent_representation(
        ...
        for tensors in scdl:
            ...
            tensors = {
                k: v.to(device) if isinstance(v, torch.Tensor) else v for k, v in tensors.items()
            }
    ...
    def predict(
        ...
        predictions = self._format_predictions(predictions, prediction_mode, soft, adata, indices)
    ...
    def _compute_predictions(
        ...
        if soft:
            ...
            if prediction_mode in ["l", "label"]:
                filtered_mapping = self._get_label_mapping_for_predictions()
                if filtered_mapping is not None and len(filtered_mapping) < pred.shape[1]:
